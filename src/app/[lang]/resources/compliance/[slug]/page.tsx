...
import { getComplianceBySlug } from "@/data/loader";
...
export default async function ComplianceDetailPage({
    ...
    const response = await getComplianceBySlug(slug, lang);
    const resourceData = response?.data?.[0] || response?.data;

    if (!resourceData) notFound();

    const { title, tag, description, date, read, content } = resourceData;
            ...
            <nav className="pt-24 pb-8 px-6 md:px-10 border-b border-gray-100">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-2 text-xs font-medium text-gray-400">
                        ...
                        <span className="text-[#013228] font-bold">Compliance</span>
                    </div>
                </div>
            </nav>
